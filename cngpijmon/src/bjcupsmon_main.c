...
PRIVATE ST_ErrorCodeTbl	gErrorCodeTbl[] = {	// Error code table.
	...
	//Ver.3.70 for STA
	{ID_ERR__SLEEP,					ID_ERR_LEVEL_CONTINUE,	ID_OTHER_MESSAGE_SLEEP},
	{ID_ERR__BUSY,					ID_ERR_LEVEL_CONTINUE,	ID_OTHER_MESSAGE_BUSY},
	{ID_ERR__FIRMUP,				ID_ERR_LEVEL_CONTINUE,	ID_OTHER_MESSAGE_FIRMUP},
	{ID_ERR__OTHEROPERAION,			ID_ERR_LEVEL_CONTINUE,	ID_OTHER_MESSAGE_OTHEROPERATION},
	{ID_ERR__OFFLINE,				ID_ERR_LEVEL_CONTINUE,	ID_OTHER_MESSAGE_OFFLINE},
	{ID_ERR__USB_BACKEND_NORMAL,	ID_ERR_LEVEL_CONTINUE,	ID_OTHER_MESSAGE_USB_BACKEND_NORMAL},
};


/*** Static function prototypes ***/
PRIVATE gint initApplication(gint, gchar**, gchar*);
PRIVATE gboolean updateUI(gint);
...
//	gint			cnt = 0;					// Counter.
	...
	gchar	backend_name_normal_usb[] =	"usb:/";	/* Normal USB Backend Name */
	gchar	macaddress[128];						/* macaddress */
	gint	backend_Is = -1;
	gchar	modelIDstr[5];
	gchar	devpath[MAX_BUF_SIZE];					/* /dev/usb/lp0 etc */
	gint	ext_errorLevel = -1;
/*** Parameters end ***/
...
//	signal( SIGTERM, cnijnpr_sigterm_handler );
	...
	if (retVal != ID_ERR_NO_ERROR){
		ext_errorLevel = ID_ERR_LEVEL_FATAL;
		goto EXT_SPOOLER_ERROR;
		//exit(0) ;
	}

	/* get devpath */
	if( strlen(deviceURI) != 0 ){
		strncpy(devpath, deviceURI + strlen(backend_name_usb)-1, (strlen(deviceURI)-strlen(backend_name_usb)+1) );
	}

	/* get Mac Address */
	/* When cnijnet is Backend */
	if (strncmp(deviceURI, backend_name_net, strlen(backend_name_net)) != 0){
		/* When cnijusb is Backend */
		...
		/*  usb backend  */
		else if(strncmp(deviceURI, backend_name_normal_usb, strlen(backend_name_normal_usb)) == 0) {
			backend_Is = USB_BACKEND_NORMAL;
		}
		/*  usb backend  */
	...
	/* child process */
		...
		char	backend_type[256];						/* username(loginnamae) */
		int		rtn = 0;
		...
		memset( backend_type, 0x00, sizeof(backend_type) ) ;
		//printf("Child --%d\n", getpid());	
		...
		sprintf( backend_type, "%d", backend_Is );
		...
		/* Ver.3.70 for USB */
		cmd_argv[cmd_argv_cnt++] = backend_type;
		cmd_argv[cmd_argv_cnt++] = devpath;
		...
		if (retVal == ID_ERR_NO_ERROR) {
			...
			if (retVal == ID_ERR_NO_ERROR) {
				...
				else if( (backend_Is == USB_BACKEND_NORMAL) && (mismatch_flag != 1) ){
					strncpy(windowTitle, productName, sizeof(windowTitle)-strlen(STR_USB_NAME)-1 );
					strncat(windowTitle, " ", strlen(" "));
					strncat(windowTitle, STR_USB_NAME, strlen(STR_USB_NAME));
				}
				...
				if (retVal == ID_ERR_NO_ERROR) {
					...
					if ( backend_Is == USB_BACKEND_NORMAL ) {
						retVal = ID_ERR__USB_BACKEND_NORMAL;
					}
					else {
...
EXT_SPOOLER_ERROR:
		if (retVal != ID_ERR_NO_ERROR) {
			...
			getErrorCodeInfo(retVal, &errorInfo);
			/* force FATAL error */
			if(ext_errorLevel == ID_ERR_LEVEL_FATAL) errorInfo.errorLevel = ID_ERR_LEVEL_FATAL;
			if (errorInfo.errorLevel != ID_ERR_LEVEL_FATAL) {
...
///////////////////////////////////////////////////////////////////////////////////////////
// 
// CS     : PRIVATE gboolean updateUI(backend_Is)
// IN     : None.
// OUT    : None.
// RETURN : Always TRUE.
// 
PRIVATE gboolean updateUI(gint backend_Is)
//for Network Ver.3.00
...
/*** Parameters end ***/
	...
	if (retVal == ID_ERR_NO_ERROR) {
		...
		/* Ver.3.70 for IPP */
		if ( (jobExist == FALSE) && !( (backend_Is == USB_BACKEND_NORMAL) || (backend_Is == CANON_USB_BACKEND_CNIJ_USB) || (backend_Is == CANON_BJNP_BACKEND_CNIJ_NET) ) ) {
		//if ( (jobExist == FALSE) && !( (backend_Is == CANON_USB_BACKEND_CNIJ_USB) || (backend_Is == CANON_BJNP_BACKEND_CNIJ_NET) ) ) {
		//if ( (jobExist == FALSE) && (*small_name == '\0') ) {
			retVal = ID_ERR_PRINT_JOB_NOT_EXIST;
		}
		/* must ID_ERR__USB_BACKEND_NORMAL */
		else if(backend_Is == USB_BACKEND_NORMAL){
			retVal = ID_ERR__USB_BACKEND_NORMAL;
		}
		else {
			...
			// Ver.3.10 for Network
			/* Ver.3.70 */
			if(  (backend_Is == CANON_BJNP_BACKEND_CNIJ_NET) || (backend_Is == CANON_USB_BACKEND_CNIJ_USB) ){
			//if( *small_name != '\0' ){
				//read from stdin
				//check stdin
				{
					...
					/* stdin (fd 0) OK? */
					select_flag = select(1, &rfds, NULL, NULL, &tv);
				...
				/* Ver.3.70 */
				/* usually read, but not copy */
				/* if( (backend_Is == CANON_BJNP_BACKEND_CNIJ_NET) || (backend_Is == CANON_USB_BACKEND_CNIJ_USB) ){ */
					if( buf[0] == '@' ){
						retVal = ID_ERR__NO_RESPONSE;
					}
					else if( buf[0] == '?' ){
						retVal = ID_ERR__OFFLINE;
					}
					else if( buf[0] == '*' ){
						retVal = ID_ERR__COLLECTING_STATUS;
					}
					else if( buf[0] == 's' ){
						retVal = ID_ERR__SLEEP;
					}
					else if( buf[0] == 'w' ){
						retVal = ID_ERR__BUSY;
					}
					else if( buf[0] == 'f' ){
						retVal = ID_ERR__FIRMUP;
					}
					else if( buf[0] == 'o' ){
						retVal = ID_ERR__OTHEROPERAION;
					}
			...
			//if (retVal == ID_ERR_NO_ERROR) {
			if ( (retVal == ID_ERR_NO_ERROR) && (select_flag == 1) ) {
				// Check status string.
				if (strncmp(statusStr, STR_HEADER_STATUS_STRING, strlen(STR_HEADER_STATUS_STRING)) == 0) {
					// Initialize printer status area.
